...
def cynical_frog(start_y, start_x, home_y, home_x):
    ...
    while bfs_list:
        ...
        for i in range(4):
            ...
            if 0 <= x < map_x and 0 <= y < map_y and visited_map[y][x] == INF and super_jump_enable == True:
        ...
        if super_jump_enable == True:
            ...
            for i in range(0, map_y):
                if jump_visited_map[i][cur_x] == INF and move + 1 < visited_map[i][cur_x]:
                    ...
                    jump_visited_map[i][cur_x] = move+1
...
pond = list()
visited_map = [[INF for _ in range(map_x)]for __ in range(map_y)]
jump_visited_map = [[INF for _ in range(map_x)]for __ in range(map_y)]
