...
# 이 경우 운동 1일차에 대학원생은 중량이 3만큼 증가하지만 그와 동시에 하루에 중량이 4만큼 감소하기 때문에, 1일이 지난 이후에 중량은 499가 된다.
# 따라서 조건을 만족하지 못한다.
...
def excercise(kits: list, samdae):
    global count
    if kits == []: count += 1; return
    for i in range(len(kits)):
        gain = kits[i]
        if (samdae + gain) < 0: continue
        samdae += kits.pop(i)
        excercise(kits, samdae)
        kits.insert(i, gain)
        samdae -= gain
    return


days, loss = map(int, input().split(" "))
kits = list(map(int, input().split(" ")))
for i in range(days): kits[i] -= loss
