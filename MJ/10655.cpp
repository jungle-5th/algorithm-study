...
// 문제
// 농장에 있는 젖소들이 건강하지 못하다고 생각한 농부 존은 젖소들을 위한 마라톤 대회를 열었고,
// 농부 존의 총애를 받는 젖소 박승원 역시 이 대회에 참가할 예정이다.
// 마라톤 코스는 N (3 ≤ N ≤ 100000) 개의 체크포인트로 구성되어 있으며,
...
int p1[2];
int p2[2];
cin >> p0[0] >> p0[1] >> p1[0] >> p1[1] >> p2[0] >> p2[1];
int honest = abs(p1[0]-p0[0]) + abs(p1[1]-p0[1]);
int cheat_before = abs(p2[0]-p0[0]) + abs(p2[1]-p0[1]);
int cheat_now;

for (int i = 3; i < checkpoints; i++) {
    p0[0] = p1[0];
    p0[1] = p1[1];
    p1[0] = p2[0];
    p1[1] = p2[1];
    cin >> p2[0] >> p2[1];
    cheat_now = honest + abs(p0[0]-p2[0]) + abs(p0[1]-p2[1]);
    cheat_before = min(cheat_before + abs(p1[0]-p2[0]) + abs(p1[1]-p2[1]), cheat_now);
    honest += abs(p0[0]-p1[0]) + abs(p0[1]-p1[1]);
}
cout << cheat_before;
