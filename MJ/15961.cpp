...
int main() {
//////////////////////////////////
int n_of_dish, d_of_kind, k_of_streak, coupon;
cin >> n_of_dish >> d_of_kind >> k_of_streak >> coupon;

vector<int> kindCheck(d_of_kind+1, 0);
queue<int> streakList;
vector<int> firstFilled(k_of_streak);
kindCheck[coupon] = 1;
int combo = 1;
int fish;
// first fill
for (int k = 0; k < k_of_streak; k++) {
    cin >> fish;
    if (kindCheck[fish] == 0) combo++;
    kindCheck[fish]++;
    streakList.push(fish);
    firstFilled[k] = fish;
}
...
int maxcombo = combo;
// second, check one by one
int top;
for (int n = k_of_streak; n < n_of_dish; n++) {
    top = streakList.front();
    cin >> fish;
    kindCheck[top]--;
    streakList.pop();
    if (kindCheck[top] == 0) {
