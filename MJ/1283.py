...
nOfOptions = int(input())
options = []
for _ in range(nOfOptions):
    options.append(list(input().strip().split()))
for option in options:
    registered = False
    for i in range(len(option)):
        word = option[i]
        if not alphabet[makeOrd(word[0])]:
            alphabet[makeOrd(word[0])] = True
            res = list(word)
            res.insert(1,"]")
            res.insert(0,"[")
            option[i] = "".join(res)
            ...
            break
    if not registered:
        for w in range(len(option)):
            word = option[w]
            for i in range(len(word)):
                if not alphabet[makeOrd(word[i])]:
                    alphabet[makeOrd(word[i])] = True
                    res = list(word)
                    res.insert(i+1, "]")
                    res.insert(i, "[")
                    option[w] = "".join(res)
