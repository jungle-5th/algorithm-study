...
int main() {
//////////////////////////////////////////////
int nOfNations, m; cin >> nOfNations >> m;
vector<vector<int>> scores(nOfNations+1, vector<int>(3,0));
for(int n = 1; n <= nOfNations; n++) {
    int nation; cin >> nation;
    cin >> scores[nation][0] >> scores[nation][1] >> scores[nation][2];
}
int gold = scores[m][0], silver = scores[m][1], bronze = scores[m][2];
int rank = 1;
for(int n = 1; n <= nOfNations; n++) {
    if(scores[n][0] > gold) {rank++; continue;}
    else if(scores[n][0] == gold) {
        if(scores[n][1] > silver) {rank++; continue;}
        else if(scores[n][1] == silver) {
            if(scores[n][2] > bronze) rank++;
        }
    }
}
cout << rank;
