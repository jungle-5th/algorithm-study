...
# 예를 들어, 전투력 10,000 이하의 캐릭터는 WEAK, 10,000 초과 그리고 100,000 이하의 캐릭터는 NORMAL,
...
nOfTitle, mOfPlayer = map(int, input().split())
titles = []
steps = []
for n in range(nOfTitle):
    title = list(input().strip().split())
    titles.append(title[0])
    steps.append(int(title[1]))

for m in range(mOfPlayer):
    print(titles[bisect.bisect_left(steps,int(input()))])