...
sumCosts = [0]
for cost in costs:
    sumCosts.append(sumCosts[-1]+cost)
if sumCosts[-1] <= maxCost: print(costs[-1]); exit()

costLimit = maxCost//n
while(True):
    index = bisect.bisect_left(costs,costLimit)
    costLeft = maxCost-sumCosts[index]-costLimit*(n-index)
    if costLeft >=0: costLimit +=1
    else:print(costLimit-1);break
