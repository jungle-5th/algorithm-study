...
while (true){
    ...
    int jaStreak = 0, moStreak = 0;
    if (pw[0] == 'a' || pw[0] == 'e' || pw[0] == 'i' || pw[0] == 'o' || pw[0] == 'u') {moStreak = 1; noVowel=false;}
    else jaStreak = 1;
    
    for (int i = 1; i < pw.size(); i++) {
        if (pw[i] == pw[i-1]) {
            if (pw[i] != 'e' && pw[i] != 'o') {streakError = true; break;}
        }
        if (pw[i] == 'a' || pw[i] == 'e' || pw[i] == 'i' || pw[i] == 'o' || pw[i] == 'u') {
            jaStreak = 0;
            moStreak += 1;
            noVowel = false;
            if (moStreak > 2) {streakError = true; break;}
        }
        else {
            jaStreak += 1;
            moStreak = 0;
            if (jaStreak > 2) {streakError = true; break;}
        }
    }
    cout << "<" << pw << "> is ";
