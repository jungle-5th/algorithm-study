...
str2 = []
m_of_inputs = int(sys.stdin.readline())

for _ in range(m_of_inputs):
    order = sys.stdin.readline().strip()
    if order == 'L':
        if len(str1) > 0: str2.append(str1.pop(-1))
    elif order == 'D':
        if len(str2) > 0: str1.append(str2.pop(-1))
    elif order == 'B':
        if len(str1) > 0: str1.pop(-1)
    else: str1.append(order.split(" ")[1])
