...
bool descending(const vector<int>& a, const vector<int>& b) {
    return a[0] > b[0];
}
...
vector<vector<int>> homeworks(n_of_hw, vector<int>(2));
for (int n = 0; n < n_of_hw; n++) cin >> homeworks[n][1] >> homeworks[n][0];
sort(homeworks.begin(), homeworks.end(), descending);
int lastday = homeworks[0][0];
for (int n = 0; n < n_of_hw; n++) {
    if (lastday > homeworks[n][0]) lastday = homeworks[n][0];
    lastday -= homeworks[n][1];
}
cout << lastday;
/////////////////
}