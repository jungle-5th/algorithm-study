...
// 각각의 테스트 케이스의 첫째 줄에 처음에 놓여있는 카드의 개수 N(1 ≤ N ≤ 1,000)이 주어진다. 두 번째 줄에는 N장의 카드에 적힌 알파벳의 초기 순서가 주어진다.
// 가장 왼쪽에 있는 카드에 적혀있는 알파벳부터 순서대로 N개가 공백으로 구분되어 주어진다. 모든 카드에는 한 개씩의 알파벳이 적혀있으며, 모두 대문자이다. 
...
for (int t = 0; t < testcase; t++) {
    ...
    char alphabet;
    cin >> str;
    for (int n = 1; n < nofcards; n++) {
        cin >> alphabet;
        if (str[0] < alphabet) str = str + alphabet;
        else str = alphabet + str;
    }
    for (int n = 0; n < nofcards; n++) cout << str[n];
    cout << endl;
}
/////////////////////////////////////////////
return 0;}